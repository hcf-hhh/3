  ...
  <div>
    <el-form class="manager-form">
      <el-form-item label="设备电量">
        <el-input style="width: 200px;margin-left: 10px;float: left;" v-model="powerTest"
                  placeholder="请输入设备电量"></el-input>
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <span v-if="scope.row.powerTest < 60" style="color: red;">{{ scope.row.powerTest }}</span>
            <span v-else>{{ scope.row.powerTest }}</span>
          </template>
        </el-table-column>
        <el-table-column
            prop="countTest"
...
export default {
  ...
  methods: {
    ...
    searchList() {
      DeviceService.findByPowerTest(this.powerTest).then(res => {
        this.tableData = Array.from(res.data).splice(
            (this.page - 1) * this.size,
            this.size
        )
